...
class ApexAuthClient:
    ...
    def _refresh_token(self) -> str:
        ...
        headers = {"x-api-key": self._config.x_api_key}
...
class ApexHTTPClient:
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Any:
        ...
        logger.info(f"API Response: {method} {endpoint} | Status: {response.status_code} | Time: {duration:.2f}s")
    ...
    def _handle_response_errors(response: requests.Response) -> None:
        ...
        if not response.ok:
            try:
                ...
                msg = f"Erro {response.status_code}: {error_detail}"
            except Exception:
